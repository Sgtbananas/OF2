...
from core.logger import log_error  # Assume logger.py provides log_error
...
def backtest_strategy(df: pd.DataFrame, signals: pd.Series, symbol: str, target: float):
    ...
    if df.empty or signals.empty:
        msg = f"❌ No data or signals to backtest for {symbol}"
        print(msg)
        log_error(msg)
    ...
    try:
        ...
        if not trades:
            msg = f"⚠️ No trades executed for {symbol}."
            print(msg)
            log_error(msg)
            return {"symbol": symbol, "pnl": 0.0, "win_rate": 0.0, "trades": []}
        ...
        err_msg = f"❌ Exception during backtest for {symbol}: {e}"
        print(err_msg)
        log_error(err_msg)
        return {"symbol": symbol, "pnl": 0.0, "win_rate": 0.0, "trades": []}
